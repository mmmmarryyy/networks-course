...
1. Какой IP-адрес и номер порта TCP использует ваш компьютер (отправитель), передающий
   ...
   - IP-адрес: `192.168.0.9`, номер порта TCP: `50246`
...
3. Какой порядковый номер у SYN TCP-сегмента, который используется для установления
   ...
   - raw is `3840824295`, если про *relative* порядковый номер, то `0`
   - можем увидеть SYN flag, говорящий, что это SYN-сегмент
...
4. Какой порядковый номер у SYNACK-сегмента, отправленного сервером gaia.cs.umass.edu
   ...
   - опять же raw - `483378738`, *relative* порядковый номер, естественно, `0`
   - значение raw поля acknowledgement number равно `3840824295`, *relative*  поля acknowledgement number равно `1`
   - сервер прибавил к relative порядковому номеру единицу
   - установлен acknowledgement flag
...
6. Рассмотрите TCP-сегмент, содержащий команду POST протокола HTTP, как первый TCP-сегмент 
   ...
   - порядковые номера у первых шести сегментов TCP-соединения:
      - raw - `3840824296`, *relative* - `1`
      - raw - `3840824930`, *relative* - `635`
      - raw - `3840826390`, *relative* - `2095`
      - raw - `3840827850`, *relative* - `3555`
      - raw - `3840829310`, *relative* - `5015`
      - raw - `3840830770`, *relative* - `6475`
   ...
   - RTT для каждого:
      1. через `15.655844 - 15.429515 = 0.226329`c
      2. через `15.655845 - 15.429577 = 0.226268`c
      3. через `15.676127 - 15.429582 = 0.246545`c
      4. через `15.676129 - 15.429586 = 0.246543`c
      5. через `15.676129 - 15.429588 = 0.246541`c
      6. через `15.676130 - 15.429592 = 0.246538`c
7. Чему равна пропускная способность (количество байтов, передаваемых в единицу
   ...
   - `152138` Б / (`15.928338`c - `15.429515`c) = `304993.956` б/c
   - Размер alice.txt равен 152138 байт, деленный на разницу времен между первым пакетом данных и последним ACK пакетом

### Работа с Time-Sequence-Graph (Stevens) (2 балла) - done
...
#### Решение
$$
0.98 \leq \dfrac{n \cdot L/R}{RTT + L/R} = \dfrac{n \cdot 0.012}{30 + 0.012} \Longrightarrow\\
n \geq \dfrac{0.98 \cdot 30.012}{0.012} = 2450.98 \approx 2451
$$
